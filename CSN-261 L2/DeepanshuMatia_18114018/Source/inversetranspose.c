...
* @brief The program decrypts an encrypted text file using a given encryption key
...
int main(int argc, char *argv[])
{
    ...
    if (n < 1){
        printf("%s\n", "Decryption is not possible for this value of n.");
        return 0;
    }
    ...
    //! Creating a new string that contains the decrypted code.
    char str2[length];
    for (int j=0; j < length/n ;j++ ){
    for (int i = 0; i < n; i++)
    {
    	str2[(a*i+b)%n + n*j] = str[i + n*j];
    }
}

    //! Creating output file that contains the decrypted code.
    FILE* fptr;
